...
EXTENSIONS = {".dart", ".yaml", ".yml", ".xml", ".gradle", ".kts", ".properties"}
...
def copy_to_clipboard(text: str) -> bool:
    try:
        ...
        if system == "windows":
            process = subprocess.run(["clip"], input=text.encode("utf-16le"), check=False)
...
def main(default_project: str | None = None) -> int:
    ...
    if copied:
        ...
        output_name = f"dump_flutter_{base.name}_{datetime.now():%Y%m%d_%H%M%S}.txt"
